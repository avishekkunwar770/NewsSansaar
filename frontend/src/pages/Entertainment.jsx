...
const Entertainment = () => {
  ...
  useEffect(() => {
    // Fetching the entertainment news
    fetch("http://localhost:5000/api/news/category/entertainment")
      .then((res) => res.json())
      .then((result) => {
        setEntertainmentNews(result.data || []);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <div className="pt-24 px-6">
      ...
      <h1 className="text-4xl font-bold mt-4">Entertainment</h1>
      <p className="text-lg text-gray-500 mb-6">
        Latest news and updates from the entertainment category.
      </p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {entertainmentNews.map((news) => (
            <div
              key={news.id} // Using news.id for better key management
              className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition duration-300"
            >
              {/* Display news thumbnail if available */}
              {news.thumbnail && (
                <img
              ...
              <div className="p-4">
                <h2
                  ...
                  onClick={() => navigate(`/foreign?id=${news.id}`)} // Change this to redirect to the foreign route
                >
                  {news.title}
                </h2>
